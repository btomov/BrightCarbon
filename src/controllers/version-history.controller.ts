...
export const versionHistoryController = {
  async getVersions(
    ...
    try {
      ...
      if (!versions.length) {
        return res
          .status(404)
          .json({ error: "No versions found for the specified note" });
      }
  ...
  async restoreVersion(
    ...
    try {
      const version = await VersionHistory.findById(
      ...
      return res
        .status(200)
        .json({ message: "Note restored successfully", note });
    } catch (error) {
      return handleError(res, error);
    }
  },
  ...
  async deleteVersions(
    ...
    try {
      const result = await VersionHistory.deleteMany({
      ...
      return res
        .status(200)
        .json({ message: "All versions deleted successfully" });
    } catch (error) {
      return handleError(res, error);
    }
  },
